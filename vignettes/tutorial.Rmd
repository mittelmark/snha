...
abstract: >
    ...
    interactiving variables.
    In this package vignette the basic workflow for analyzing raw data or for 
    analysing correlation matrices is demonstrated.
...
It is as well possible to install the latest version directly from the Guithub
...
* *snha_get_chains* - to get the actual chains which are found and which build the graph
* *snha_rsquare* - get r-square values for the nodes based on linear model to have a qualitative measure for the graph predeiction. 
* _plot.snha_ - plot an snha graph object
