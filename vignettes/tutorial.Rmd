...
We can as well extract the distances between all nodes, so their shortest path
to other nodes using the *mgraph_shortest_paths* method like this:

```{r label=spath,results='asis'}
knitr::kable(mgraph_shortest_paths(as))
```

More important nodes have short distance to all other nodes. There are several
centrality measures available for graphs. One example is the harmonic
centrality which can be applied on a graph like this:

```{r results='asis'}
knitr::kable(data.frame(nodes=rownames(as$theta),
hc=round(mgraph_harmonic_centrality(as),2)))
```
 

As you can see the nodes 'lwt' and 'bwt' have the highest centrality values, whereas the random data node 'rnd' has a centrality value of zero.

We discuss this centrality topic this later in more detail.
...
For the  evaluation of the  importance of a node in a network often  centrality
measures  like  degree,  closeness,  betweenness  or eigen  vector  centrality
measures  are  used.  In  the  package  there  are  two  centrality   measures
implemented,  the degree  centrality, the number of edges  connected to a node
and the harmonic  centrality (Marchiori and Latora 2000 @Marchiori2000).  The latter is implemented as the  normalized  sum of the reciprocal
shortest  path  lengths of a note to all other  nodes in the  network.  So the
formula is as follows:

![](equation.svg)

Where  the  harmonic  mean  _h_ of a node  _u_ is  defined  as the  sum of the
reciprocal of all shortest path  distances (d) divdied by N, the number of nodes,
- 1.

This  centrality  measure can be as well used to colorize more important, more
central nodes differentially then others. Here an example:

```{r fig=TRUE,fig.height=6,fig.width=10}
par(mai=c(0.2,0.2,0.2,0.2))
B = mgraph(type="barabasi",nodes=20,edges=3)
mgraph_harmonic_centrality(B)
cols=mgraph_nodeColors(B,type="harmonic")
lg =mgraph_nodeColors(B,type="harmonic",legend=TRUE)
sizes = 4 + 6 * mgraph_harmonic_centrality(B)
plot(B,layout="sam",vertex.color=cols,vertex.size=sizes)
legend("bottom",fill=lg$col, legend=lg$legend,ncol=6)
```

For more information on harmomic  centrality have a look at this blog entry by
by [symbio6.nl](https://symbio6.nl/en/blog/analysis/harmonic-centrality).
